...
function StartDownloadMail($link, $filename)
{
    ...
    echo "File4Aria_End is $file4aria";
    echo '<br/>';
    ...
    $command = "nohup \"{$aria2c}\" --file-allocation=none --check-certificate=false --max-connection-per-server=10 --split=10 --max-concurrent-downloads=10 --summary-interval=0 --continue --user-agent=\"Mozilla/5.0 (compatible; Firefox/3.6; Linux)\" --input-file=\"{$file4aria}\"  2>&1 &";
    echo $command;
    echo "<br/>";
    exec("{$command}");
    @unlink($file4aria);
